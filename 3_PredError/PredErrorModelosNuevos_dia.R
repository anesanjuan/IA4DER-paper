...
paths <- c(
  ...
  "NuevosResultados/PrediccionErrorNuevo/PrediccionMAPE/REDUCIDO_DIA"
)
...
fwrite(df, "REDUCIDO_DIA/preds_MAPE_reducido.csv")


###############

df <- fread("REDUCIDO_DIA/preds_MAPE_reducido.csv")
...
for (target in mape_targets) {
  ...
  resultados_base <- df[, .(id, dia, real, target_value = get(target))]
  ...
  tune_and_train <- function(learner_id, search_space, file_name, model_name, extra_params = list()) {
    ...
    tryCatch({
      ...
      fwrite(archive_dt, paste0("REDUCIDO_DIA/Archive_", model_name, "_", target, ".csv"))
      
      saveRDS(at, paste0("REDUCIDO_DIA/Modelo_", model_name, "_", target, ".rds"))
  ...
  ), paste0("REDUCIDO_DIA/Pred_", target, "_rf.csv"), "rf")
  ...
  ), paste0("REDUCIDO_DIA/Pred_", target, "_xgboost.csv"), "xgboost")
  
  # SVM
  #tune_and_train("regr.svm", ps(
  #  cost = p_dbl(0.1, 10),
  #  gamma = p_dbl(0.01, 0.1)
  #), paste0("REDUCIDO_DIA/Pred_", target, "_svm.csv"), "svm",
  #extra_params = list(type = "eps-regression", kernel = "radial"))
  
  # KNN
  tune_and_train("regr.kknn", ps(
    k = p_int(3, 15)
  ), paste0("REDUCIDO_DIA/Pred_", target, "_knn.csv"), "knn")
  
  # Linear Regression (sin tuning)
  learner_lm <- lrn("regr.lm")
  learner_lm$train(task)
  saveRDS(learner_lm, paste0("REDUCIDO_DIA/Modelo_lm_", target, ".rds"))
  ...
  fwrite(resultados_lm, paste0("REDUCIDO_DIA/Pred_", target, "_lm.csv"))
}
