...
fwrite(df, "REDUCIDO_DIA_HORA/preds_MAPE_reducido.csv")


###############

df <- fread("REDUCIDO_DIA_HORA/preds_MAPE_reducido.csv")
...
for (target in mape_targets) {
  ...
  tune_and_train <- function(learner_id, search_space, file_name, model_name, extra_params = list()) {
    ...
    tryCatch({
      ...
      fwrite(archive_dt, paste0("REDUCIDO_DIA_HORA/Archive_", model_name, "_", target, ".csv"))
      
      saveRDS(at, paste0("REDUCIDO_DIA_HORA/Modelo_", model_name, "_", target, ".rds"))
  ...
  ), paste0("REDUCIDO_DIA_HORA/Pred_", target, "_rf.csv"), "rf")
  ...
  ), paste0("REDUCIDO_DIA_HORA/Pred_", target, "_xgboost.csv"), "xgboost")
  ...
  tune_and_train("regr.kknn", ps(
    k = p_int(3, 15)
  ), paste0("REDUCIDO_DIA_HORA/Pred_", target, "_knn.csv"), "knn")
  
  # Linear Regression (sin tuning)
  learner_lm <- lrn("regr.lm")
  learner_lm$train(task)
  saveRDS(learner_lm, paste0("REDUCIDO_DIA_HORA/Modelo_lm_", target, ".rds"))
  ...
  fwrite(resultados_lm, paste0("REDUCIDO_DIA_HORA/Pred_", target, "_lm.csv"))
}
